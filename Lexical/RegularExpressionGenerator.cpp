...
#include "Regulars.h"
#include "RegularExpressionGenerator.h"

using namespace std;

/*
 * The implementation of "RegularExpressionGenerator".
 */

// First stage
// create expression with symbols or transitions only
bool RegularExpressionGenerator::firstPhase(const vector<StringToken>& lineTokens){

	// "conc" variable used to keep track where should we add the concatenation symbol.
	// if "conc" = 0 that's mean if new symbol found we don't put the "!" sign => that could happen after
	//"-" or "|" as they are binary operations.
...
RegularExpressionGenerator::RegularExpressionGenerator(unordered_map<std::string,Regular>& defs) : userDefinedRegulars(defs){

}

bool RegularExpressionGenerator::generateRegularExpression(const vector<StringToken>& lTokens, bool isUserDefinedRegulars = false){
	
	//call the three functions sequntially
	bool tryToSolve = firstPhase(lTokens) && secondPhase() && thirdPhase();
	...
	if(isUserDefinedRegulars){
		definitions[lTokens[0].getData()] = defintnion;
	}
...
RegularExpression RegularExpressionGenerator::getProductionRegularExpression(){
